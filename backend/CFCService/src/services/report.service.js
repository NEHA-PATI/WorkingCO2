exports.generateReport = (total, breakdown) => {
  ...
  // =========================
  // =========================
// 💡 ADVANCED DIVERSE RECOMMENDATION ENGINE
// =========================
...
// ---------- FLIGHTS ----------
if (flights > 0) {
  const flightRoutes = Object.entries(breakdown.flights || {})
    .sort((a, b) => b[1] - a[1]);

  const [topRoute, topRouteEmission] = flightRoutes[0];

  recommendationCandidates.push({
    text: `Your route "${topRoute}" is your largest single contributor (~${Math.round(topRouteEmission)} kg CO₂). Replacing one similar flight with train or virtual meetings would significantly reduce emissions.`,
...
if (electricity > 0) {
  ...
  recommendationCandidates.push({
    text: `Improving home efficiency (LEDs, AC optimization, reducing standby power) could cut ~${Math.round(savings)} kg CO₂ monthly.`,
    savings,
    category: "housing"
  });
}
...
if (carEmission > 0) {
  ...
  recommendationCandidates.push({
    text: `Carpooling, hybrid driving, or using public transport 1–2 days per week could save ~${Math.round(savings)} kg CO₂.`,
    savings,
    category: "transport"
  });
}
...
if (chicken > 0) {
  ...
  recommendationCandidates.push({
    text: `Replacing half of your chicken meals with plant-based alternatives could reduce ~${Math.round(savings)} kg CO₂.`,
    savings,
    category: "food"
  });
}
...
if (flights > housing && flights > transport) {
  summary = "Air travel is the dominant driver of your footprint. Reducing flight frequency will have the largest impact.";
} else if (housing > flights && housing > transport) {
  summary = "Home energy consumption is your primary emission source. Efficiency improvements can significantly lower your footprint.";
} else if (transport > flights && transport > housing) {
  summary = "Personal vehicle usage is a key contributor. Mobility changes can meaningfully reduce emissions.";
} else {
  summary = "Your emissions are distributed across multiple categories. Balanced improvements across lifestyle areas will help reduce your footprint.";
}
